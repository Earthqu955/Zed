...
حقـوق للتـاريخ 
@RR8R3 - كتـابـة الملـف :  زلــزال الهيبــه"""
...
ZelzalVP_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝘇𝗮𝗶𝗱𝗵𝗼𝗻 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/Xl441) 𓆪\n\n"
    ...
    "\n𓆩 [𝙕𝙚𝙙 𝙑𝙖𝙧𝙨 - قنـاة الفـارات](t.me/zzzzl5) 𓆪"
)
...
async def variable(event):
    ...
    # All Rights Reserved for "Zed-Thon" " الهيبـه"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر {} بنجـاح ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت زيـد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
...
async def variable(event):
    ...
    # All Rights Reserved for "Zed-Thon" " الهيبـة"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        variable = "ALIVE_TEMPLATE"
        await asyncio.sleep(1.5)
        if gvarstatus("ALIVE_TEMPLATE") is None:
        	return await zed.edit("**✾╎عـذراً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**✾╎تم حـذف {} بنجـاح ☑️**\n**✾╎المتغيـر المحـذوف : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت زيـد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def custom_zed(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        if variable is None:
            ...
            await zed.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎قيمـة الفـار : ↶**\n `{}` \n\n**✾╎قنـاة السـورس : @Xl441**".format(input_str, variable))
...
async def variable(event):
    ...
    vmsr = "Africa/Cairo"
    heroku_var = app.config()
    zed = await edit_or_reply(event, "**✾╎جـاري أعـداد المنطقـه الزمنيـه لـ زيـدثــون 🌐...**")
    # All Rights Reserved for "Zed-Thon" " الهيبـة"
    if input_str == "العراق" or input_str == "عراق":
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المتغير : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت زيـد ثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
    ...
    if malatha.reply_to_msg_id:
        ...
        if input_str in ["الحماية", "الحمايه"]:
            ...
            try:
                ...
                await zed.edit("**✾╎تم تغييـر صـورة {} .. بنجـاح ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n\n**✾╎قنـاة السـورس : @Xl441**".format(input_str, vinfo))
