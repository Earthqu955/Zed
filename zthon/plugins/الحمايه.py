...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""𝗭𝗔𝗜𝗗𝗛𝗢𝗡™𓅛 - 𝐙𝐀𝐈𝐃𝐇𝐎𝐍𓆪
◐━─━─━─━─𝗭𝗔𝗜𝗗𝗛𝗢─━─━─━─━◐
...
**⤶ فقط قل سبب مجيئك او اختـر احـد الخيـارات بالاسفـل ⬇️ وانتظـر الـرد 🧸♥️**"""
    else:
        USER_BOT_NO_WARN = f"""𓆩zᵃᶤᵈʰᵒᶰ - 𝑷𝑴 𝗭𝗔𝗜𝗗𝗛𝗢𝗡
◐━─━─━─━─𝙕𝞝𝘿─━─━─━─━◐
...
async def do_pm_options_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = "**⤶ اخـتـر احـد الخـيـارات بــدون تـڪـرار ، وهـذا هــو تـحـذيـرڪ الاخـيـر 🌪️**"
...
async def on_new_private_message(event):
    ...
    zel_dev = (5776497081, 5776497081, 1885375980, 1721284724, 1951523146, 1243462298, 1037828349, 1985711199, 2028523456, 2045039090, 1764272868, 2067387667, 294317157, 2066568220, 1403932655, 1389046667, 444672531, 2055451976, 294317157, 2134101721, 1719023510, 1985225531, 2107283646, 2146086267, 1850533212, 5280339206)
    ...
    if event.chat_id in zel_dev:
        ...
        return await event.client.send_message(chat, "**⪼ اطـلق هـلاو احد المطوࢪيـن المساعدين هنا اننـي محظـوظ لقدومـك الـي 🙈♥️𐏕**")
    if event.chat_id == 5375407655 or event.chat_id == 5375407655 or event.chat_id == 5776497081:
        reason = "**- انـه مطـوࢪ السـورس زالـزال الهـيبة  🥳♥️𓆰**"
        try:
            PM_WARNS = sql.get_collection("pmwarns").json
        except AttributeError:
            PM_WARNS = {}
...
async def pmpermit_on(event):
    ...
    if input_str == "تفعيل":
        if gvarstatus("pmpermit") is None:
            ...
            await edit_delete(event, "** ⌔∮ امـر حمايـه الخـاص بالفعـل .. مُفعـل  🧸♥️**")
    elif gvarstatus("pmpermit") is not None:
        delgvar("pmpermit")
        await edit_delete(
            event, "**⌔∮تـم تعطيـل أمـر حمايـة الخـاص .. بنجـاح 🧸💔...**"
        )
    else:
    ...
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:
            ...
            await edit_delete(
                event,
                "**⌔∮تـم تعطيـل أمـر حمايـة الخـاص .. بنجـاح 🥇...**",
            )
        else:
            await edit_delete(
        ...
        await edit_delete(
            event, "** ⌔∮ امـر حمايـه الخـاص بالفعـل .. مُفعـل  🧸♥️**"
        )
...
async def pmpermit_on(event):
    ...
    if input_str == "تعطيل":
        ...
        delgvar("pmmenu")
        await edit_delete(
            event, "**⌔∮تـم تفعيـل امـر حمايـه الخـاص .. بنجـاح 🧸💔...**"
        )
    else:
        await edit_delete(
...
async def approve_p_m(event):  # sourcery no-metrics
    ...
    if not pmpermit_sql.is_approved(user.id):
        ...
        await edit_delete(
            event,
            f"⌔∮ [{user.first_name}](tg://user?id={user.id})\n**⌔∮تـم السـمـاح لـه بـإرسـال الـرسـائـل 🧸♥️** \n **⌔∮ الـسـبـب ❔  :** {reason}",
        )
        try:
            PMMESSAGE_CACHE = sql.get_collection("pmmessagecache").json
        except AttributeError:
